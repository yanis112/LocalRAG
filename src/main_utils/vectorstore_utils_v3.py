        ...
        with tqdm(total=len(pending_files), desc="Processing new files") as pbar:
            for name, full_path in pending_files:
                ...
                if result:
                    ...
                    self.processed_docs_data[name] = {"id":len(self.processed_docs_data), "subfolder_path":subfolder_path}
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.delete_ids or args.delete_paths or args.delete_folders:
            ids_to_delete = [int(id) for id in args.delete_ids] if args.delete_ids else None
